...
async function getDb() {
  ...
  dbReady = (async () => {
    // Simple connection test + schema init using pool directly
    const pool = getPool();
    try {
...
async function initSchema(pool) {
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS users (id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), email TEXT UNIQUE NOT NULL, password_hash TEXT NOT NULL, display_name TEXT NOT NULL, tenant_id UUID NOT NULL, created_at BIGINT NOT NULL)`);
  await pool.query(`CREATE TABLE IF NOT EXISTS refresh_tokens (token TEXT PRIMARY KEY, user_id UUID NOT NULL, tenant_id UUID NOT NULL, expires_at BIGINT NOT NULL, created_at BIGINT NOT NULL)`);
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS scenarios (id SERIAL PRIMARY KEY, name TEXT NOT NULL, type TEXT NOT NULL DEFAULT 'ALL_PROPERTIES', target_id TEXT, description TEXT, created_at BIGINT NOT NULL, tenant_id UUID NOT NULL, sync_version BIGINT NOT NULL DEFAULT 0, deleted INTEGER NOT NULL DEFAULT 0)`);
  await pool.query(`CREATE TABLE IF NOT EXISTS rule_scenario_relation (rule_id INTEGER NOT NULL, scenario_id INTEGER NOT NULL, tenant_id UUID NOT NULL, PRIMARY KEY (rule_id, scenario_id))`);
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS sync_checkpoints (tenant_id UUID PRIMARY KEY, last_sync_time BIGINT NOT NULL DEFAULT 0, sync_token TEXT, is_syncing INTEGER NOT NULL DEFAULT 0, last_error TEXT)`);
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS backup_records (id SERIAL PRIMARY KEY, tenant_id UUID NOT NULL, device_name TEXT NOT NULL DEFAULT '', app_version TEXT NOT NULL DEFAULT '', data_json TEXT NOT NULL, data_size INTEGER NOT NULL DEFAULT 0, checksum TEXT NOT NULL DEFAULT '', created_at BIGINT NOT NULL)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_kr_tenant ON keyword_rules(tenant_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_am_tenant ON ai_model_configs(tenant_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_rh_tenant ON reply_history(tenant_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_mb_tenant ON message_blacklist(tenant_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_backup_tenant ON backup_records(tenant_id)`);
