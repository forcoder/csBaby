...
async function exec(sql, params = []) {
  try {
    ...
    // 检测连接问题，强制重连
    if (e.message.includes('cannot commit') || e.message.includes('connection') || e.code === 'ECONNREFUSED' || e.code === 'ETIMEDOUT') {
      console.error('Connection error detected, forcing reconnect');
      poolVersion++;
    }
