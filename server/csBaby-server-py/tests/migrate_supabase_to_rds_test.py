...
def mock_pymysql(monkeypatch):
    """模拟 pymysql.connect"""
    mock = MagicMock()
    monkeypatch.setattr('pymysql.connect', mock)
    return mock
...
class TestMigration:
    ...
    def test_migrate_table_with_data(self, mock_source_conn, mock_target_conn, monkeypatch):
        """正常场景9: 迁移有数据的表"""
        # 模拟 pymysql.extras.execute_batch
        # 不再需要 mock execute_batch（已改为逐行执行）
...
class TestExceptionCases:
    ...
    def test_connect_with_invalid_url(self, mock_pymysql):
        """异常1: 连接无效 URL"""
        mock_pymysql.side_effect = Exception('could not connect to server')
